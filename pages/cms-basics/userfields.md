...
| ------ | ------ | ---------- | ------------------- |
| Главный модуль | `USER` | Пользователь

*  {% rowspan=4 colwidth=[192] %}

   1С-Битрикс: Управление сайтом

   Коробочная версия Битрикс24 |
| Блоги | `BLOG_BLOG` | Блог |
| Блоги | `BLOG_POST` | Сообщение в блоге |
| Блоги | `BLOG_COMMENT` | Комментарий сообщения |
| Задачи | `TASKS_TASK` | Задача

*  {% rowspan=5 colwidth=[192] %}

   Коробочная версия Битрикс24 |
| Задачи | `TASKS_SCRUM_ITEM` | Элемент скрама |
| Задачи | `TASKS_TASK_TEMPLATE_CHECKLIST` | Чек-лист в шаблоне |
| Задачи | `TASKS_TASK_CHECKLIST` | Чек-лист в задаче |
| Задачи | `TASKS_TASK_TEMPLATE` | Шаблон задачи |
| Информационные блоки | `IBLOCK_N_SECTION` | Раздел инфоблока с `ID = N`
...
*  {% rowspan=2 colwidth=[192] %}
   ...
   Коробочная версия Битрикс24 |
| Информационные блоки | `IBLOCK_N` | Инфоблок с `ID = N` |
| Календарь | `CALENDAR_EVENT` | Событие календаря | Коробочная версия Битрикс24 |
| Обучение | `LEARN_ATTEMPT` | Попытка в тесте

*  {% rowspan=14 colwidth=[192] %}

   1С-Битрикс: Управление сайтом

   Коробочная версия Битрикс24 |
| Социальная сеть | `SONET_GROUP` | Группа соцсети |
| Социальная сеть | `SONET_COMMENT` | Комментарий |
| Социальная сеть | `SONET_LOG` | Лог |
| Форум | `FORUM_MESSAGE` | Сообщение форума |
| Highload-блоки | `HLBLOCK_N` | Highload-блок с `ID = N` |
| Торговый каталог | `PRODUCT` | Товар |
| Торговый каталог | `CAT_STORE` | Склад |
| Торговый каталог | `CAT_STORE_DOCUMENT_A` | Документ складского учета *Приход товара на склад* |
| Торговый каталог | `CAT_STORE_DOCUMENT_S` | Документ складского учета *Оприходование товара* |
| Торговый каталог | `CAT_STORE_DOCUMENT_M` | Документ складского учета *Перемещение товара между складами* |
| Торговый каталог | `CAT_STORE_DOCUMENT_R` | Документ складского учета *Возврат товара* |
| Торговый каталог | `CAT_STORE_DOCUMENT_D` | Документ складского учета *Списание товара* |
| Торговый каталог | `CAT_STORE_DOCUMENT_U` | Документ складского учета *Отмена резервирования* |
| Корзина | `RECYCLEBIN_DISK` | Элемент корзины

*  {% rowspan=23 colwidth=[192] %}

   Коробочная версия Битрикс24 |
| CRM | `CRM_MAIL_TEMPLATE` | Шаблон письма |
| CRM | `CRM_TIMELINE` | Таймлайн |
| CRM | `CRM_LEAD` | Лид |
| CRM | `CRM_DEAL` | Сделка |
| CRM | `CRM_COMPANY` | Компания |
| CRM | `CRM_CONTACT` | Контакт |
| CRM | `CRM_ORDER` | Заказ |
| CRM | `CRM_INVOICE` | Счет |
| CRM | `CRM_смарт-процесс` | Смарт-процесс |
| CRM | `CRM_ACTIVITY` | Действие бизнес-процесса |
| CRM | `CRM_QUOTE` | Коммерческое предложение |
| CRM | `CRM_LEAD_SPD` | Лид с привязкой к элементам корзины |
| CRM | `CRM_DEAL_SPD` | Сделка с привязкой к элементам корзины |
| CRM | `CRM_COMPANY_SPD` | Компания с привязкой к элементам корзины |
| CRM | `CRM_CONTACT_SPD` | Контакт с привязкой к элементам корзины |
| CRM | `CRM_ORDER_SPD` | Заказ с привязкой к элементам корзины |
| CRM | `CRM_INVOICE_SPD` | Счет с привязкой к элементам корзины |
| CRM | `CRM_смарт-процесс_SPD` | Смарт-процесс с привязкой к элементам корзины |
| CRM | `CRM_ACTIVITY_SPD` | Действие БП с привязкой к элементам корзины |
| CRM | `CRM_QUOTE_SPD` | Коммерческое предложение с привязкой к элементам корзины |
| Роботизация бизнеса (RPA) | `RPA_id процесса` | Робот |
| Роботизация бизнеса (RPA) | `RPA_COMMENT` | Комментарий к роботу |
