...
Когда системных типов недостаточно, можно создать собственный тип поля. Как описать класс типа, зарегистрировать его и связать с компонентом отрисовки, читайте в статье [Типы и компоненты пользовательских полей в главном модуле](./userfields-main-types).
...
1. На странице *Настройки > Настройки продукта > Пользовательские поля* нажмите Добавить.
   ...
   ![](./_images/userfields.png){width=717px height=455px}
...
2. В форме редактирования объекта, где доступны пользовательские поля, перейдите по ссылке *Добавить пользовательское свойство*. Например, в форме редактирования пользователя, раздела инфоблока, блога или сущности CRM.
   ...
   ![](./_images/userfields-2.png){width=664px height=354px}
...
![](./_images/userfields-3.png){width=700px height=615px}
...
-  `OnUserTypeBuildList` — вызывается при построении списка пользовательских полей методом `CUserTypeManager::GetUserType`. Через это событие регистрируют [собственные типы полей](./userfields-main-types#зарегистрировать-тип-поля).
