...
**Пример.** Минимальная разметка текстового поля выглядит так.
...
**Пример.** Вместе эти классы дают поле с заголовком, действием и подсказкой.
...
Если форме нужна готовая сетка из строк с подписями слева, ее задает отдельное расширение [ui.layout-form](./ui-layout-form.md). Оно зависит от `ui.forms` и добавляет классы строк, подписей и секций формы.
...
**Пример.** Поле поиска с иконкой слева и кнопкой очистки справа собирают так.
...
**Пример.** Многострочное поле занимает всю доступную ширину, а менять его размер нельзя.
...
**Пример.** Подпись поля выбора файла выглядит как кнопка.
...
Область `ui-ctl-file-drop` получает только оформление. Прием перетащенных файлов и события `dragover` и `drop` обрабатывает код формы. Готовый механизм приема описан в статье [Загрузка файлов Uploader](./uploader.md).
...
**Пример.** Поле показывает ключ доступа, а группа справа собирает две кнопки — показать значение и скопировать его. Кнопки оформляет расширение [ui.buttons](./ui-buttons.md).
...
**Пример.** Поле с текстом ошибки под ним размечают так.
...
-  [Сетка формы ui.layout-form](./ui-layout-form.md) — строки, подписи и секции формы вокруг полей `ui-ctl`.
