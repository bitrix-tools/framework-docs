...
От языка BitrixSetup зависят доступные продукты и дистрибутив для загрузки. Скрипт выбирает язык по настройкам браузера.

Для лицензии российской линейки выберите русский язык. Например, редакция «Корпоративный портал» доступна только в русской версии BitrixSetup. Если русского языка нет в списке, добавьте параметр `?lang=ru` к адресу скрипта: `http://[адрес_сайта]/bitrixsetup.php?lang=ru`

Перед загрузкой коммерческой версии убедитесь, что редакция соответствует ключу. При ошибке `403 Доступ запрещен` проверьте язык, редакцию и ключ.
...
![](./_images/install-distr-2.png){width=733px height=291px}
...
![](./_images/install-distr-3.png){width=726px height=843px}
...
![](./_images/install-distr-4.png){width=600px height=379px}
...
![](./_images/install-distr.png){width=600px height=163px}

### Параметры администратора

Заполните поля, чтобы создать учетную запись администратора.

![](./_images/install-distr-5.png){width=600px height=302px}
