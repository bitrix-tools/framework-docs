...
-  `createFromText($text)` — разбирает текст по словарям текущего языка сайта и возвращает объект `DateTime`. Вернет `null`, если не сможет распознать дату. Метод понимает числовые даты и словесные выражения на языке сайта, например, `вчера`, `завтра` для русского или `yesterday`, `tomorrow` для английского.

    Если в тексте есть дата, но нет времени, метод вернет дату со временем окончания рабочего дня из настроек модуля Календарь, а не с текущим временем. Если опция не задана, используется время по умолчанию — `18:00`.

    ```php
    use Bitrix\Main\Type\Date;

    $date = Date::createFromText('вчера'); // Для сайта на русском языке
    echo $date?->format('d.m.Y H:i:s'); // Вчерашняя дата со временем окончания рабочего дня
    ```

    ```php
    use Bitrix\Main\Type\Date;

    $date = Date::createFromText('yesterday'); // Для сайта на английском языке
    echo $date?->format('d.m.Y H:i:s'); // Вчерашняя дата со временем окончания рабочего дня
    ```
    ...
    $date = Date::createFromText('25.11.2025 10:30'); // Точные дата и время
    echo $date?->format('d.m.Y H:i:s'); // 25.11.2025 10:30:00
    ```

    ```php
    use Bitrix\Main\Type\Date;

    $date = Date::createFromText('25.11.2025');
    echo $date?->format('d.m.Y H:i:s'); // 25.11.2025 19:00:00, если рабочий день заканчивается в 19:00
    ```
...
Метод `toString()` преобразует объект даты в строку с учетом региональных настроек сайта.
...
### Методы конвертации времени {#methods}
