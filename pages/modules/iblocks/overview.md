...
![](./_images/overview-5.png){width=370px height=462px}
...
Подробнее в статье [Компоненты](./../../framework/components.md)
...
Подробнее в разделе [ORM](./../../orm/orm-concepts.md)
...
![](./_images/overview-2.png){width=778px height=135px}
...
-  Одиночные свойства представляются как отношение `Reference`.

-  Множественные свойства представляются как отношение `OneToMany`.

![](./_images/overview-3.png){width=481px height=210px}
...
![](./_images/overview-4.png){width=543px height=108px}
...
2. Версия `2` — значения свойств хранятся в выделенных таблицах инфоблока. Одиночные свойства и кешированные значения множественных свойств хранятся в таблице `b_iblock_element_prop_s{IBLOCK_ID}`, где `{IBLOCK_ID}` — идентификатор инфоблока. Значения множественных свойств хранятся в таблице `b_iblock_element_prop_m{IBLOCK_ID}`. Выборка элементов со всеми свойствами выполняется быстро. Не рекомендуется, если количество свойств превышает 50.
