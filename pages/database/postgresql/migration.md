...
1. Создайте пользователя и базу PostgreSQL.
   ...
   ```bash prompt="$"
   $ sudo -u postgres createuser bitrix
   $ sudo -u postgres psql -c 'grant create on schema public to "bitrix"'
   GRANT
   $ sudo -u postgres createdb portaldb --owner bitrix --lc-ctype C.UTF-8 --template=template0
   $ sudo -u postgres psql -d portaldb -c 'CREATE EXTENSION IF NOT EXISTS pgcrypto'
   CREATE EXTENSION
   $ sudo -u postgres psql -d portaldb -c 'ALTER USER "bitrix" WITH PASSWORD '\''passwd'\'''
   ALTER ROLE
   ```
...
3. Сделайте дамп базы данных MySQL.
   ...
   ```bash prompt="$"
   $ mysqldump --opt --skip-extended-insert --hex-blob -u root portaldb > /tmp/mysql_dump.sql
   ```
4. Сконвертируйте его в PostgreSQL.

   ```bash prompt="$"
   $ php -f bitrix/modules/perfmon/tools/mysql_to_pgsql.php -- --mysqldump=/tmp/mysql_dump.sql > /tmp/pgsql_dump.sql
   ```
5. Убедитесь, что все получилось.

   ```bash prompt="$"
   $ less /tmp/pgsql_dump.sql
   ```
6. Добавьте дамп в PostgreSQL.

   ```bash prompt="$"
   $ sudo -u www-data psql -b -q --user bitrix -d portaldb -f /tmp/pgsql_dump.sql
   ```
7. Добавьте дополнительные функции в PostgreSQL.

   ```bash prompt="$"
   $ grep -v 'ALTER TABLE b_group' bitrix/modules/main/install/pgsql/install_add.sql | sudo -u www-data psql -b -q --user bitrix -d portaldb
   ```
