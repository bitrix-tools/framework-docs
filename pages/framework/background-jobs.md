...
-  Метод `addAgent` добавляет агент.
   ...
   CAgent::AddAgent(
       ...
       "mymodule",           // Модуль. Пустое значение используют только в старом коде без модуля
...
![](./_images/background-jobs.png){width=638px height=340px}
...
1. Откройте страницу *Настройки > Инструменты > Командная PHP-строка.* Выполните код:
   ...
   Если скрипт вывел `Z`, это означает, что параметр отсутствует в конфигурации базы данных. Такая ситуация может возникнуть при нестандартной ситуации или повреждении настроек модуля.
...
1. Откройте страницу *Настройки > Инструменты > Командная PHP-строка*. Выполните код, который полностью отключит запуск агентов.
   ...
   Если один из символов, которые скрипт вывел `Z`, это означает, что параметр отсутствует в конфигурации базы данных. Такая ситуация может возникнуть при нестандартной ситуации или повреждении настроек модуля.
...
Для нового кода используйте собственный модуль. Файл `init.php` подключается на раннем этапе пролога, поэтому ошибка в файле может нарушить работу сайта.
