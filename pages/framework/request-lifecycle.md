...
AJAX-контроллер обрабатывает запросы напрямую через PHP-файл. Маршрутизация не активируется, шапка и подвал сайта не подключаются. Это позволяет работать только с нужными данными в фоне и обновлять часть страницы без полной перезагрузки.
