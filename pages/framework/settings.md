...
Подробнее в статье [Роутинг](./routing#configuration).
...
Секция `smtp` включает локальные SMTP-подключения и позволяет задать SMTP-сервер по умолчанию. Настройка доступна с версии Главного модуля 21.900.0.

#### Включение локальных SMTP-подключений

Добавьте секцию `smtp` в файл `/bitrix/.settings.php`, прежде чем создавать SMTP-подключения в административном интерфейсе. Параметр `enabled` обязателен: без него система не использует созданные подключения для отправки писем.

```php
'smtp' => [
    'value' => [
        'enabled' => true, // Включить отправку через SMTP
        'debug' => true, // Записывать обмен данными с SMTP-сервером в лог
        'log_file' => '/home/bitrix/www/bitrix/mailer.log', // Пример пути для BitrixVM
    ],
    'readonly' => true,
],
```
Параметры `debug` и `log_file` необязательны. Каталог для лога должен существовать, а пользователь, от имени которого работает PHP, должен иметь право на запись. Если не указывать `log_file`, система запишет лог в файл `mailer.log` в корне сайта `DOCUMENT_ROOT`.

Настройте SMTP-подключения в административном интерфейсе на странице *Настройки > Настройки продукта > Почтовые и СМС события > Настройки SMTP*.

{% note warning "" %}

Используйте защищенное соединение SMTPS или STARTTLS. Сертификат SMTP-сервера должен быть действительным, выданным доверенным центром сертификации и соответствовать имени сервера. Самоподписанный сертификат не пройдет проверку при установке защищенного соединения.

{% endnote %}

#### Настройка SMTP-сервера по умолчанию

Чтобы отправлять письма через один SMTP-сервер по умолчанию, укажите параметры подключения в секции `smtp`:
...
'smtp' => [
    'value' => [
        ...
        'encryption_type' => 'smtps', // SMTPS для порта 465, STARTTLS для других портов
    ],
    'readonly' => true,
],
```

Для отладки SMTP-сервера по умолчанию добавьте параметры `debug` и `logFile` в тот же массив `value`:
...
'smtp' => [
    'value' => [
        // ...
        'debug' => true,
        'logFile' => '/home/bitrix/www/bitrix/mailer.log', // Пример пути для BitrixVM
    ],
    'readonly' => true,
],
```

Если не указывать `logFile`, система запишет лог в файл `mailer.log` в корне сайта `DOCUMENT_ROOT`.
