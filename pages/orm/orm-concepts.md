...
-  `StringField` — поле для коротких строк.
   ...
   По умолчанию хранит строку до 255 символов. Параметр `size` задает максимальную длину: значение может быть меньше или больше 255.
...
-  `TextField` — поле для текста, который не нужно ограничивать размером `StringField`.
   ...
   Параметр `long` определяет тип хранения. Если параметр не указан или равен `false`, поле хранится как `text`. Если указать `long => true`, поле хранится как `longtext`.

   Конструктор:

   ```php
   new Entity\TextField('NAME', [
       'long' => true, // Использовать longtext вместо text (по умолчанию false)
   ]);
   ```
...
![](./_images/orm-concepts-2.png){width=664px height=178px}
